...
async def get_price_endpoint(request: PriceRequest):
    try:
        result = priceAgent.graph.invoke({"messages": [HumanMessage(content=request.user_input)]})
        ...
        str_output = ""
        for product, prices in json_output.items():
            original_price = float(str(prices[0]).strip("$"))
            discounted_price = float(str(prices[1]).strip("$"))
            database.execute_query(product, original_price, discounted_price)
            str_output += f"The product is {product}, whose original price is ${original_price:.2f} and discounted price is ${discounted_price:.2f}\n"

        return {"results": str_output}
